...
// -- Main
module.exports = function(ES6Pakket, libname, version, type) {
  describe('ES6Pakket introspection:', () => {
    ...
    describe('Test ES6Pakket constructor:', () => {
      if (type === 'with new') {
        it('Expects ES6Pakket() without the operator "new" to throw an error.', () => {
          try {
            ES6Pakket();
          } catch (e) {
            expect(e.message).to.be.a('string').that.is.equal('ES6Pakket needs to be called with the new keyword!');
          }
        });
      }

      const o = type === 'with new' ? new ES6Pakket() : ES6Pakket();
      ...
      it(`Expects ES6Pakket object to own ${1 + OWNPROPS} property(ies).`, () => {
      ...
      describe('Check the inherited generic properties:', () => {
        it(`Expects ES6Pakket object to inherit ${1 + INHPROPS} property(ies).`, () => {
